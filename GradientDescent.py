...
def GradientDescent(theta0, theta1, mileage, LearningRate, km, price, m):
	...
	while (True):
		if math.isnan(theta1) or math.isnan(theta0):
			print("OGINB !")
			sys.exit(1)
		tmp_theta0 = LearningRate * (1 / m * SumTheta0(theta0, theta1, km, price, m))
		tmp_theta1 = LearningRate * (1 / m * SumTheta1(theta0, theta1, km, price, m))
		# print(tmp_theta0)
		# print(tmp_theta1)
		if math.isnan(tmp_theta1) or math.isnan(tmp_theta0):
			print("BINGO !")
		if abs(tmp_theta0) < float(0.001) and abs(tmp_theta1) < float(0.001):
